...
const tag = "Popular";
const tagProductos = menu.filter(p => p.tag === tag).slice(0, 4);
    ...
    <div className="bg-creamMid xs:min-h-[calc(100dvh-4rem)] md:py-26 py-16 flex flex-col items-center justify-center md:gap-16 gap-12 px-6">
      ...
      <div className="flex flex-col items-center gap-3 text-center">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] uppercase tracking-[0.25em] text-darkWarm/90">Productos {tag}</span>
          <span className="w-8 h-px bg-darkWarm/25" />
        </div>
        <h2 className="text-darkWarm font-titleText text-center text-3xl md:text-6xl leading-tight">
          Solo en Central
        </h2>
      </div>
        ...
        {tagProductos.map((item, idx) => (
          <Link href={`/menu/${item.slug}`} className="grid grid-cols-2 xs:grid-cols-1 gap-2 xs:gap-1 group" key={item.slug}>
            <div className="relative w-full rounded-primarySize aspect-square overflow-hidden shrink-0 bg-primary grid place-items-center col-span-2">
              ...
              />
              {/* Badge firma */}
              <div aria-hidden="true" className="absolute top-3 left-3 flex items-center gap-1 bg-darkWarm text-primary text-[9px] uppercase tracking-widest px-2 py-1 rounded-full">
